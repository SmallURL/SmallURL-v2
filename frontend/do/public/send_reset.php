...
if (!isset($_POST['hash']) || $_POST['hash'] == "") {
	// Missing hash!
	header('Location: ../?p=forgot_password&i=1');
	die();
}
else {
	...
	if ($correct_hash !== $user_hash) {
		header('Location: ../?p=forgot_password&i=2');
		die();
	}
}
if (!isset($_POST['email']) || $_POST['email'] == "") {
	header('Location: ../?p=forgot_password&i=3');
	die();
}
...
if (count($data) == 1) {
	...
	if ($user['passreset'] == "1") {
		...
		header('Location: ../?p=forgot_password&i=4');
		die();
	}
	else {
	...
	header('Location: ../?p=forgot_password&i=5');
}
?>