...
$rcount = db::get_array("entries",array("custom"=>true,"user"=>$_SMALLURL['UID']),"COUNT(*)");
...
<table class="table">
	...
	<?php
	$randoms = $pagination->get("entries",array("custom"=>true,"user"=>$_SMALLURL['UID']),$page,$perpage);
	if (count($randoms) > 0) {
...
<ul class="pagination">
	...
	<?php if ($page != 0) { ?>
		<li><a href="javascript:load_page('#customurls','<?=$_SERVER['SCRIPT_NAME']?>',0);">&laquo;</a></li>
	<?php } else { ?>
...
if ($page > 0) {
	$prevpage = $page-1;
	$prevlink = "javascript:load_page('#customurls','{$_SERVER['SCRIPT_NAME']}',{$prevpage});";
	?>
...
for ($i=$page-3; $i!=$page; $i++) {
	if ($i > 0) {
		$ln = "javascript:load_page('#customurls','{$_SERVER['SCRIPT_NAME']}',{$i});";
...
if ($page < $pcount) {
	$nextpage = $page+1;
	$nextlink = "javascript:load_page('#customurls','{$_SERVER['SCRIPT_NAME']}',{$nextpage});";
	?>
...
<?php if ($page != $pcount) { ?>
	<li><a href="javascript:load_page('#customurls','<?=$_SERVER['SCRIPT_NAME']?>',<?=$pcount?>);">&raquo;</a></li>
<?php } else { ?>
